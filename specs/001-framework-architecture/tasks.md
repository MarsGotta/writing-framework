...
- [X] T062 [US3] Extend `.claude/skills/writeonmars-redaccion/SKILL.md` with a `--parallel N` mode: dispatches up to N sub-agents concurrently using Claude Code's parallel Agent tool calls (single message, multiple tool uses). Each sub-agent receives the same shared context (brief, temario, glosario, descripciones contiguas) and writes to its dedicated chapter file (no shared writes).
- [X] T063 [US3] Extend `.claude/skills/writeonmars-contraste/SKILL.md` with parallel-per-chapter dispatch (each chapter's contrast runs in its own sub-agent), then consolidates findings into `findings.md`. Conflicts in citation interpretations between chapters surface as a consolidated note.
- [X] T064 [US3] Enhance `.claude/skills/writeonmars-glossary/SKILL.md` collision detection for parallel ingestion: detects when two chapters introduce the same term with divergent definitions, flags it as `critico`, blocks consolidation until resolved (FR-015 edge case from spec).
- [X] T064a [US3] [Test] Establecer baseline serial de 4 capítulos sobre el mismo tema del piloto US3 ejecutando `writeonmars-redaccion` en modo secuencial (sin `--parallel`); cronometrar y archivar evidencia en `tests/editorial-pilot/evidence/<YYYY-MM-DD>-pilot-parallel-baseline/`. Capturar métricas: tiempo total, hallazgos críticos por pasada, cobertura de glosario.
- [X] T065 [US3] [Test] Pilot a 4-chapter guide with 2 chapters drafted in parallel via `--parallel 2` sobre el mismo tema y manifiesto que T064a. Cronometrar y capturar las mismas métricas de calidad (críticos/pasada, cobertura de glosario).
- [X] T066 [US3] [Test] Validate SC-006: paralelo ≥40% más rápido que el baseline T064a **y** equivalencia ±10% en (a) hallazgos críticos por pasada y (b) cobertura de glosario. Document evidence in `tests/editorial-pilot/evidence/<YYYY-MM-DD>-pilot-parallel/parallel-validation.md`.
- [X] T067 [P] [US3] Create `docs/parallel-execution.md`: when to use parallel mode, recommended N values, glossary collision handling and known limitations.
...
- [X] T068 [US4] Create `.claude/skills/writeonmars-update/SKILL.md`: compares the project manifest skill versions against the canonical repo's skills, presents a diff of what would change, and applies the update on confirmation. Preserves project-local configuration (manifest fields the user customised).
- [X] T069 [US4] Create `docs/maintenance/skill-update-procedure.md`: step-by-step procedure for updating a bundled skill (version bump → vault sync → update entry in compatibility-matrix → run smoke tests → release notes).
- [X] T070 [US4] Create `docs/maintenance/constitution-update-procedure.md`: step-by-step procedure for amending the constitution and propagating templates per the existing "Procedimiento de enmienda" of the constitution itself, plus how installed projects pick up the change.
- [X] T071 [US4] [Test] Create `tests/smoke/update-skill-on-installed-project.sh`: installs the framework into a temp repo, bumps the version of a bundled skill in the canonical source, runs `writeonmars-update`, and asserts the project's manifest reflects the new version while a custom field set by the operator survives. Validates SC-008.
- [X] T072 [US4] Document the time-to-update result in `docs/maintenance/skill-update-procedure.md` § "Performance" to validate SC-008 (<15 minutes).
...
- [X] T073 [P] Implement `mcp/writeonmars-research/server.py` (FR-009b): Python 3.11 reference implementation that wraps web search + fetch + local resources into the citation contract; emits citation records validated against `contracts/citation-record.schema.json`. Include `mcp/writeonmars-research/pyproject.toml` and `mcp/writeonmars-research/README.md`.
- [X] T074 [P] Create `agents/codex/` adapter scaffolding mirroring `agents/claude/`: empty prompt files matching the canonical names, plus a `README.md` explaining what needs to be filled in to support Codex. Demonstrates the agnosticism declared in FR-023/FR-024 even if not fully wired in v1.
- [X] T075 [P] Document SC-007 portability readiness in `docs/portability-validation.md`: (a) qué partes del harness son agente-agnósticas (skills nucleares en `.claude/skills/`, contratos publicados, manifiesto), (b) qué partes requieren adaptador (`agents/<name>/prompts/`), (c) checklist de pasos para portar una skill a Codex/Cursor cuando exista soporte. SC-007 v1 = entrega de scaffolding + contratos; la validación end-to-end con un segundo agente se difiere a feature posterior.
- [X] T076 [P] Create `docs/contributing.md`: how to propose a new skill, how to certify a new MCP for the citation contract (link `contracts/citation-contract.md` § "Cómo certificar"), how to contribute to `resources/` and how to amend the constitution.
- [X] T076a [P] Document the optional external vector memory in `docs/memory-external.md`: esquema mínimo (entidad, fuente, fecha, etiquetas) per constitution § "Arquitectura del framework"; procedimiento de activación (`manifest.memory_external.enabled: true`, `provider`, `uri`); procedimiento de reconstrucción desde artefactos del repo (FR-022); aviso explícito de que la memoria es caché, nunca fuente de verdad. Cross-link a `contracts/manifest-schema.json` § `memory_external`.
- [X] T077 Run the full `quickstart.md` end-to-end on a fresh test repository; record results in `tests/editorial-pilot/evidence/<YYYY-MM-DD>-quickstart/quickstart-validation.md`. Confirms the documented path matches reality after all phases.
- [X] T078 [P] Polish `README.md`: pitch, architecture diagram (text), quickstart link, status of each user story, current version.
- [X] T079 Archive a snapshot of `.specify/memory/constitution.md` (v1.1.0) into `releases/v1.0.0/constitution-v1.1.0.md` for historical reference. Naming aclara que el framework v1.0.0 ratifica la constitución v1.1.0; futuras releases pueden mantener la misma constitución sin renombrar.
- [X] T080 Tag `v1.0.0` once all checkpoints pass and update `CHANGELOG.md` with the user-story-by-user-story summary. (CHANGELOG preparado; tag manual pendiente del mantenedor.)
