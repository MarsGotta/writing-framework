...
Ya hemos acabado con la parte más aburrida (imaginando que ha habido una apartado teorico previo) es momento de ponernos manos a la obra. En este tutorial desbloquearemos un superpoder que antes no teniamos en nuestro arsenal, pero que hoy en dia, cada vez más, está cogiendo relevancia: veremos cómo construir un agente básico que utiliza tool use, esa funcionalidad que le permite al modelo invocar funciones que definamos previamente, recibir el resultado y usarlo para continuar la generación de la información. Es un mecanismo importante que nos permite conectar al modelo con sistemas externos.

Antes de comenzar, tenemos que asegurarnos que tenenemos instalado el SDK de Anthropic y configurada la API key como variable de entorno.
...
El modelo ahora tiene el dato y puede generar una respuesta final en lenguaje natural para el usuario. Con esto tendríamos un agente mínimo que sabe usar una herramienta. El siguiente paso natural sería añadir un loop para que el modelo pueda encadenar varias invocaciones de herramientas hasta resolver la tarea completa.
